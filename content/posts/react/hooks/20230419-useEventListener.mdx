...
React comes with severval [built-in hooks](https://react.dev/reference/react) that cover a variety of use cases. But sometimes you need a hook for a specific purpose so that you can reuse shared logic throughout your applications.
In this article, which is the first in a series showcasing my favorite react hooks I use in every project, I will be showing you both how to use the `useEventListener{:.fn}` hook and how to create your own from scatch.

The [full source code](#full-source-code) for the hook is provided at the end of the article.
...
```ts
window.addEventListner('keydown', (e: Event) => {
...
While this looks as though it should work at first glance, and it infact might for some very basic usecases, you should not do this. There are infact a few hidden pitfalls with the code shown above:
...
- Once the component is unmounted the listener will still be registered, we should instead be dilligent in cleaning up the event listener by removing it
...
```ts title="src/hooks/useEventListener.ts" showLineNumbers {1-7}
...
```ts title="src/hooks/useEventListener.ts" showLineNumbers {1-2,8-12}
import { useEffect } from 'react';
...
```ts title="src/hooks/useEventListener.ts" showLineNumbers {8-13,15} /, useRef/ /, eventListener/
...
This will prevent hyrdration errors in frameworks such as [NextJS](https://nextjs.org/) stopping your components from working in SSR.

```ts title="src/hooks/useEventListener.ts" showLineNumbers {15-17}
import { useEffect, useRef } from 'react';
...
constrained the generic type `K{:.type}` and that our listener currenty has the type `(event: any) => void{:ts}` as we don't know what event is being
listened to and thus can't properly type the event. We could naively type it as `Event{:.type}` same as we have done on line 19 and make change to `type: string{:ts}`
...
And thats all there is to it, if you have been following along then you have just created the `useEventListener(targetElement, type, listener){:ts}` custom hook from scratch! 🥳🎉
...
Thanks for taking the time to read my post, if you liked it I would greatly appreciate if you shared with your friends/colleagues. Unfortunately there is no share button,
as I haven't written that part of the blogs site yet, so you will have to do me a favour and share a link manually 😉

Any feedback is appreciated and the few socials I use are linked at the bottom of this page, I wish you a good day and happy coding!
