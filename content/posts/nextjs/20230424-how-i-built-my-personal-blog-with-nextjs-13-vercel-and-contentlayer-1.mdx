...
![{{ "width": 900, "height": 492, "alt": "Terminal Window showing 'yarn dev'", "caption": "Step 5. Starting development server" }}](/images/posts/nextjs/how-i-built-my-personal-blog-01/step5-1.webp)
...
<CalloutPanel type="info" hideType>
  ...
  running `yarn dev{:bash}` so you may need to manually run `yarn contentlayer build{:bash}` for the content to be generated.
</CalloutPanel>
...
simply by importing `allPosts{:ts}` which is an array of `Post{:.type}` from the `contentlayer/generated{:text}` path. Using this generated
array of posts we can access the data from our posts that we defined earlier such as the `title{:ts}`, `publishedDate{:ts}` and `url{:ts}`:
...
![{{ "width": 900, "height": 257, "alt": "Final result of the Feed Page" }}](/images/posts/nextjs/how-i-built-my-personal-blog-01/step5-2.png)
...
type the parameter passed from [generateStaticParams][nextjs-beta-static-params] into our `Page{:.fn}` function.
...
Secondly we define and export a function called `generateStaticParams{:.fn}`, with NextJS App Directory this is a special function that can
be used to generate dynamic route parameters at build time instead of on-demand at request time. In this case in our function we are getting
our `allPosts{:ts}` object and mapping it to `{ slug: url.split('/').slice(1) }{:ts}`, which effectively sets up all the parameters for our
page component for each of our posts. In this function we also remove the `/posts/{:text}` from the beginning of the url so the route is
correct.
...
![{{ "width": 900, "height": 235, "alt": "Demonstration of the Feed/Post pages working", "caption": " It's just that easy 🥳🎉" }}](/images/posts/nextjs/how-i-built-my-personal-blog-01/step6.webp)

We now have a working site that generates html from our markdown files, from where we can do almost anything we want to style the html such
...
All this and potentially more will be covered in future posts, as soon as they are released I will be sure to link them below!

In the meantime if you have any ideas for how you could improve this or make it your own, I highly encourage you to experiment and make
something for yourself!

If you want to see a full working example the code featured in the code blocks above is available on [GitHub][next13-blog-repo] or feel
free to take a look at the code for my personal blog which is also available on GitHub [here][this-blog-repo]
...
[next13-blog-repo]: https://DrBarnabus/next13-blog
